...
namespace PP23771.UserController
{
	public class Controller
	{
		...
		[OpenApiParameter(name: "userid", In = ParameterLocation.Path, Required = true, Type = typeof(string), Description = "exam id")]
		[OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(User), Description = "Ok response")]
		...
		[OpenApiRequestBody(contentType: "application/json", bodyType: typeof(User), Required = true, Description = "Request body")]
			    ...
			    [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "postusers")] HttpRequest req, ILogger log)
		{
			...
			List<User> users = JsonSerializer.Deserialize<List<User>>(requestBody);
